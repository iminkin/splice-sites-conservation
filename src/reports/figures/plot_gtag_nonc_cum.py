...
data = data[((data["site_type"] == "d") & (data["motif"] != "GT")) | ((data["site_type"] == "a") & (data["motif"] != "AG")) | (data["dataset"] == "Random")]
...
for tr_type_idx, (now_tr_type, tr_type_description) in enumerate(tr_types):
	for site_end, site_end_description in site_ends:
		...
		for dataset_idx, dataset in enumerate(all_datasets):
			...
			print(dataset, site_end, now_tr_type)
			print(now_df.head())
...
#			plt.ylim(0, 0.04)
		...
		plt.ylabel("Normalized number of splice sites (Cumulative)")
